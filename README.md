...
| 笔记 | 独立窗口打开语雀文档 |
| 系统 GC | 分析进程、孤儿 Helper 深度清理、后台持续观察、保护名单、48h 清理日志 |
...
- **系统托盘 → 系统 GC** 打开清理窗口
- macOS 下列出进程，标记「孤儿 Helper / 深层残留 / 高 CPU / 高内存 / 僵尸 / 保护 / 系统」，展示清理判断依据；窗口可见时每 10 秒刷新
- **普通清理**处理已由 launchd 接管、所属主应用已退出的 Helper；**深度清理**继续查找同应用的 Helper 子树，优先处理子进程，最多复扫 3 轮
- **后台自动清理**默认开启（含深度清理）：启动后 15 秒首次检查，空闲时默认每 30 分钟扫描；发现候选后每 30 秒复查，持续观察至少 60 秒才终止。长时间休眠、进程身份变化或候选消失后重新观察；扫描失败最多 5 分钟后重试
- 手动批量清理同样要求进程已运行到观察期下限，并在短暂等待后重新扫描确认；可单独手动终止指定进程
- 仅处理当前用户进程，内置保护系统路径、VsGo 自身及子进程、启动祖先；保护名单支持名称和路径片段，不区分大小写，并覆盖后代进程。高 CPU、高内存、僵尸状态本身不会触发自动终止
- 每轮发送 TERM / KILL 前复核 PID、启动时间、用户、路径及当前保护规则，给予 2 秒正常退出时间；自动清理与手动终止串行执行，关闭自动清理会取消后续自动信号
- 内存统计中的「终止前 RSS 合计」是进程占用估算，可能包含共享页，不代表系统实际释放量；系统内存条显示总量减空闲量，不等同于 macOS 内存压力
- 清理日志保留最近 48 小时、最多 2000 条（临时目录 `vsgo-gc.log`），包含观察状态、跳过原因与错误
- 回归测试：`npm run test:gc`（模拟进程快照与信号，不终止真实进程）
