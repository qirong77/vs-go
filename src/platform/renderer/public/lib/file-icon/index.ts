...
const validate = (file, options) => {
  ...
  if (!file) {
    throw new Error(
      "Specify one or an array of: app name, bundle identifier, file path, or pid",
    );
  }
  ...
  if (
    Array.isArray(file) &&
    !file.every((f) => typeof f === "string" || typeof f === "number")
  ) {
    throw new TypeError(
      "Expected all members of `file` array to be of `string` or `number` type",
    );
  }
...
const toCLIArgument = (file, { size, destination }) => {
  ...
  const toFile = (file, index) => ({
    ...toBuffer(file),
    destination: toArray(destination)[index],
  });
...
export async function fileIconToFile(file, options) {
  ...
  if (typeof file === "string" && typeof options.destination !== "string") {
    ...
    throw new TypeError(
      "Expected `file` and `options.destination` arrays to be of the same length",
    );
  }
