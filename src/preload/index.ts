...
// 全局导航栏状态管理
let navigationBarElement: HTMLDivElement | null = null;
let shadowRoot: ShadowRoot | null = null;
let isNavigationBarVisible = false;
...
function createNavigationBar() {
  // 检查是否已存在导航栏，避免重复创建
  if (navigationBarElement && document.body.contains(navigationBarElement)) {
    return;
  }

  // 创建宿主元素
  navigationBarElement = document.createElement("div");
  navigationBarElement.id = "vsgo-navigation-host";

  // 创建shadow DOM
  shadowRoot = navigationBarElement.attachShadow({ mode: "closed" });
  ...
  // 插入到页面
  document.body.insertBefore(navigationBarElement, document.body.firstChild);
...
function renderNavigationBar() {
  ...
  // 获取元素引用
  backBtn = shadowRoot.querySelector(".back-btn") as HTMLButtonElement;
  forwardBtn = shadowRoot.querySelector(".forward-btn") as HTMLButtonElement;
  refreshBtn = shadowRoot.querySelector(".refresh-btn") as HTMLButtonElement;
  urlInput = shadowRoot.querySelector(".url-input") as HTMLInputElement;
  goBtn = shadowRoot.querySelector(".go-btn") as HTMLButtonElement;
  closeBtn = shadowRoot.querySelector(".close-btn") as HTMLButtonElement;
  historyContainer = shadowRoot.querySelector(".history-container") as HTMLDivElement;
}
...
function setupHistoryEventListeners() {
  if (!urlInput || !historyContainer) return;
  ...
  displayHistory.forEach((item) => {
    ...
    historyItem.innerHTML = `
      <div style="flex: 1; min-width: 0;">
        ...
        ${item.name ? `<div class="history-item-title">${escapeHtml(item.name)}</div>` : ""}
      </div>
    `;
...
function navigateHistoryItems(direction: number) {
  if (!isHistoryVisible || !historyContainer) return;
...
function updateNavigationState() {
  if (!backBtn || !forwardBtn || !urlInput) return;
...
// 调整页面边距
function adjustBodyMargin(add: boolean) {
  if (add) {
    // 添加顶部边距
...
function hideNavigationBar() {
  if (navigationBarElement && document.body.contains(navigationBarElement)) {
    ...
    // 启动鼠标顶部检测
    startMouseTopDetection();
  }
}

// 鼠标顶部检测相关变量
let mouseTopDetectionActive = false;
let mouseTopDetectionTimeout: number | null = null;

// 启动鼠标靠近顶部的检测
function startMouseTopDetection() {
  if (mouseTopDetectionActive) return;

  mouseTopDetectionActive = true;
  document.addEventListener("mousemove", handleMouseMove);

  // 为了性能考虑，5秒后自动停止检测
  if (mouseTopDetectionTimeout) {
    clearTimeout(mouseTopDetectionTimeout);
  }
  mouseTopDetectionTimeout = window.setTimeout(() => {
    stopMouseTopDetection();
  }, 60000); // 60秒后停止检测
}

// 停止鼠标顶部检测
function stopMouseTopDetection() {
  if (!mouseTopDetectionActive) return;

  mouseTopDetectionActive = false;
  document.removeEventListener("mousemove", handleMouseMove);

  if (mouseTopDetectionTimeout) {
    clearTimeout(mouseTopDetectionTimeout);
    mouseTopDetectionTimeout = null;
  }
}
...
function setupFallbackEvents(navBar: HTMLElement) {
  ...
  function selectCurrentHistoryItem(): boolean {
    ...
    const selectedItem = historyContainer.querySelector(
      ".fallback-history-item.selected"
    ) as HTMLElement;
    if (selectedItem && selectedItem.dataset.url) {
      urlInput.value = selectedItem.dataset.url;
      hideHistory();
